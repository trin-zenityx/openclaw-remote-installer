...
const OPENCLAW_KNOWLEDGE = require('./openclaw-knowledge');
...
ROLE:
You are a flexible system administration assistant. Your primary purpose is helping install OpenClaw AI,
but you can also help with other tasks the teacher requests (checking disk space, installing other tools,
running diagnostics, uninstalling software, managing tokens, etc.).

DEFAULT INSTALLATION STEPS (for OpenClaw):
...
When the teacher sends a free-form request, help with that specific request.
When auto-install is triggered, follow the default installation steps above.
...
REFERENCE KNOWLEDGE (use this for troubleshooting and installation guidance):
${OPENCLAW_KNOWLEDGE}`;
  }

  async _callAI(session) {
  ...
  async analyzeAndSuggest(session) {
    ...
    return this._callAI(session);
  }

  async chat(message, session) {
    this.conversationMessages.push({
      role: 'user',
      content: message
    });
