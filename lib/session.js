class Session {
  ...
  isSameAgent(systemInfo) {
    if (!this.systemInfo) return false;
    return (
      this.systemInfo.hostname === systemInfo.hostname &&
      this.systemInfo.user === systemInfo.user
    );
  }

  reconnectAgent(systemInfo) {
    this.agentConnected = true;
    this.systemInfo = systemInfo; // Update in case something changed
    this.lastHeartbeat = Date.now();
    // Do NOT clear commandHistory, aiConversation, or installationState
  }
